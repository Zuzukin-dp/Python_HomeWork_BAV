...
def get_list_quotes_by_api(len_list=10):
    list_quotes = [get_quote_by_api() for _ in range(len_list)]
...
def key_reg_exp_author():
    return re.compile(r"^\d{1,2}\D{2}\s\D+\s\d{4}(.+[dD]eath|.+[bB]irthday)(.+[aA]uthor|.+[cC]reator)")


def generate_list_from_txt_file(filename="authors.txt"):
    with open(filename, "r") as f_txt:
        author_lst = [author_lst.strip() for author_lst in f_txt]
        author_lst = list(filter(key_reg_exp_author().match, author_lst))
        return author_lst
...
def key_reg_exp_date():
    return r"^\d{1,2}\D{2}\s\D+\s\d{4}"


def key_reg_exp_name():
    return r"[^\d\s-]\D+\s[A-Z]\w+['][s]"
...
def generate_dict_from_list(list_name=generate_list_from_txt_file()):
    ...
    for values in list_name:
        new_date = "".join(re.findall(key_reg_exp_date(), values))
        new_name = "".join(re.findall(key_reg_exp_name(), values))[:-2]
